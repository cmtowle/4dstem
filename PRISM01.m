...
flagAvgFP = false;
pixelSize = 0.1;            % Realspace pixel size.
emdSTEM.potBound = 2;       % Radial distance to integrate atomic potentials.
emdSTEM.numFP = 8;          % Number of frozen phonon configurations.
emdSTEM.sliceThickness = 0.5; %; % Thickness of each potential slice.
emdSTEM.interpolationFactor = [1 1]*4;

% u = ones(118,1) * 0.08;      % Debye waller coefficients.
u = ones(118,1) * 0.1;      % Debye waller coefficients.
...
emdSTEM.imageSize = round(cellDim(1:2)/pixelSize./f/4).*f*4;
